...
if ($r = mysqli_fetch_array($results)) {
    ...
    echo '<br>';
    $landid = $r['landid'];
$landlordQuery = "SELECT email FROM landlord WHERE landid = $landid";
$landlordResult = mysqli_query($conn, $landlordQuery);

if (!$landlordResult) {
    echo "Error: " . mysqli_error($conn);
} else {
    $landlordRow = mysqli_fetch_array($landlordResult);
    $landlordEmail = $landlordRow['email'];

    echo '<form method="post">';
    echo '<button type="button" class="approval-btn" onclick="approveProperty(' . $r['bookid'] . ')">approval sending</button>';
    echo '<input type="hidden" name="email" value="' . $landlordEmail . '">';
    echo '<button type="submit" class="approval-btn" name="sendEmail">Send Email</button>';
    
    echo '</form>';
}

if (isset($_POST['sendEmail'])) {
    $landlordEmail = $_POST['email'];
    $subject = 'Approval Notification';
    $message = 'Your property has been approved.';
    $headers = 'From: benjaminwell250@gmail.com';

    if (mail($landlordEmail, $subject, $message, $headers)) {
        echo '<p>Email sent to ' . $landlordEmail . '</p>';
    } else {
        echo '<p>Error sending email to ' . $landlordEmail . '</p>';
    }
}
}
?>
    ...
    <script>
        function approveProperty(propertyId) {
            ...
            if (confirmation) {
                fetch('bookingsendingappproval.php', {
                    method: 'POST',
                    headers: {
                    ...
                    .then(data => {
                        if (data.success) {
                            alert('sending email and sms successfully.');
                        } else {
